...
func NewServices(cfg config.Config, db *gorm.DB, store storage.ArtifactStore, workspace *runner.WorkspaceManager, registry *tools.ToolRegistry) *Services {
	...
	services.Hypothesis.SetClueDrivenPhase(cfg.ClueDrivenPhase)
	...
	services.ModelRuntime.SetClueDrivenPhase(cfg.ClueDrivenPhase)
	...
	services.Contract.SetDeliveryWriteback(cfg.DeliveryWriteback)
